...
type Manager interface {
	...
	Exists(tx *pgx.Tx, tableName string) bool
	// Creates a table in the database with the column names and types specified in 'colDetails'
	CreateTable(tx *pgx.Tx, tableName string, colDetails *utils.TargetColumns) error
	...
	FindColumnMismatch(tx *pgx.Tx, tableName string, colDetails *utils.TargetColumns) ([]int, error)
	...
	AddColumnsToTable(tx *pgx.Tx, tableName string, columnIndices []int, colDetails *utils.TargetColumns) error
}
...
func NewManager(schema, tableTemplate, tagTableTemplate string) Manager {
...
// Exists checks if a table with the given name already is present in the DB.
func (t *defTableManager) Exists(tx *pgx.Tx,tableName string) bool {
	if _, ok := t.Tables[tableName]; ok {
		return true
	}

	commandTag, err := tx.Exec(tableExistsTemplate, tableName, t.schema)
	if err != nil {
...
// Creates a table in the database with the column names and types specified in 'colDetails'
func (t *defTableManager) CreateTable(tx *pgx.Tx,tableName string, colDetails *utils.TargetColumns) error {
...
//     = it couldn't discover the columns of the table in the db
//     = the existing column types are incompatible with the required column types
func (t *defTableManager) FindColumnMismatch(tx *pgx.Tx,tableName string, colDetails *utils.TargetColumns) ([]int, error) {
	columnPresence, err := t.findColumnPresence(tx, tableName, colDetails.Names)
	if err != nil {
		return nil, err
	}
...
// From the column details (colDetails) of a given measurement, 'columnIndices' specifies which are missing in the DB.
// this function will add the new columns with the required data type.
func (t *defTableManager) AddColumnsToTable(tx *pgx.Tx,tableName string, columnIndices []int, colDetails *utils.TargetColumns) error {
...
func (t *defTableManager) findColumnPresence(tx *pgx.Tx,tableName string, columns []string) ([]*columnInDbDef, error) {
	existingCols, err := t.findExistingColumns(tx, tableName)
	if err != nil {
		return nil, err
	}
...
func (t *defTableManager) findExistingColumns(tx *pgx.Tx, table string) (map[string]utils.PgDataType, error) {
	rows, err := tx.Query(findExistingColumnsTemplate, t.schema, table)
	if err != nil {
